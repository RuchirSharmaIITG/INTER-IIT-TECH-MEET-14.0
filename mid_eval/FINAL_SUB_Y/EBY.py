#!/usr/bin/env python3
"""
EBY.py - Trading Strategy for EBY Dataset
...
def create_sequences(df: pd.DataFrame, feats: list, tgt: str, lb: int, hr: float = 1.0) -> tuple:
    ...
    # ==== ADDED: compute realized return R for regression model ====
    # R = (future_price / entry_price) - 1 over a fixed horizon (lookback)
    # NOTE: This is the simplest ER label — works with your existing data.
    prices = df['Price'].values
    Rf = []
    for i in fin_idx:
        entry_price = prices[i]
        exit_i = min(i + lb, len(prices) - 1)
        exit_price = prices[exit_i]
        Rf.append((exit_price / entry_price) - 1)

    Rf = np.array(Rf).astype(np.float32)
    # ==============================================================
...
def prepare_train_data(n_days):
    ...
    eby_files = sorted(eby_path.glob("day*.csv"))
    ebx_files = sorted(ebx_path.glob("day*.csv"))
    
    if len(eby_files) == 0:
        print("Error: No EBY data files found")
        return
    sel_files = []
    processed_day_tags = []
    
    if isinstance(n_days, str) and n_days.endswith('.txt'):
        print(f"\n[Loading days from {n_days}]")
        with open(n_days, 'r') as f:
            day_tags = [line.strip() for line in f if line.strip()]
        for tag in day_tags:
            parts = tag.split('_')
            ticker = parts[0]
            day_num = int(parts[1].replace('day', ''))
            if ticker == 'EBY':
                file_path = eby_path / f"day{day_num}.csv"
            elif ticker == 'EBX':
                file_path = ebx_path / f"day{day_num}.csv"
            else:
                continue
            if file_path.exists():
                sel_files.append((ticker, day_num, file_path))
        ...
        all_available_days_set = set()
        if train_days_file_path.exists():
            with open(train_days_file_path, 'r') as f:
                all_available_days_set.update(line.strip() for line in f if line.strip())
                all_available_days = list(all_available_days_set)
        ...
        print(f"\n[Preparing {len(sel_day_names)} days for training]")
        print(f"Strategy: Randomly selected days from combined EBX/EBY day lists.")

        for tag in sel_day_names:
            parts = tag.split('_')
            if len(parts) < 2:
        ...
        print(f"\nTotal days successfully mapped to files: {len(sel_files)}")
        print(f"  - EBY: {len([f for f in sel_files if f[0] == 'EBY'])}")
        print(f"  - EBX: {len([f for f in sel_files if f[0] == 'EBX'])}")
    # Process files
    print(f"\n{'='*80}")
    print("Processing Training Data")
    print(f"{'='*80}")
    
    for ticker, day_num, data_file in tqdm(sel_files, desc="Processing Days"):
        day_tag = f"{ticker}_day{day_num}"
        ...
        try:
            ...
            if len(df_proc) > CONFIG['training']['lookback']:
                ...
                out_X = out_path / f'X_{day_tag}.npy'
                out_y = out_path / f'y_{day_tag}.npy'
                
                np.save(out_X, Xt)
                np.save(out_y, yt)

                # ==== ADDED: save realized ER target ====
                out_R = out_path / f'R_{day_tag}.npy'
                np.save(out_R, Rt)
                # ========================================

                
                processed_day_tags.append(day_tag)
    ...
    if processed_day_tags:
        train_output_file = out_path / Path(CONFIG['paths']['train_days_file'])
        sorted_tags = sorted(processed_day_tags, 
                           key=lambda x: (int(x.split('_')[1].replace('day', '')), x.split('_')[0]))
        
        with open(train_output_file, 'w') as f:
            for tag in sorted_tags:
                f.write(f"{tag}\n")
        
        print(f"\n✓ Saved {len(sorted_tags)} training days to {train_output_file}")
        
        used_eby_days = {int(tag.split('_')[1].replace('day', '')) 
                        for tag in processed_day_tags if tag.startswith('EBY')}
        
        all_eby_days = set(range(0, 279))  # Days 0-278
        test_eby_days = sorted(all_eby_days - used_eby_days)
        
        test_output_file = out_path / CONFIG['paths']['test_days_file']
        with open(test_output_file, 'w') as f:
            for day in test_eby_days:
                f.write(f"day{day}\n")
        
        print(f"✓ Saved {len(test_eby_days)} test days to {test_output_file}")
        
        # Summary
        print(f"\n{'='*80}")
        print("Training Data Preparation Complete")
        print(f"{'='*80}")
        ebx_count = sum(1 for tag in sorted_tags if tag.startswith('EBX'))
        eby_count = sum(1 for tag in sorted_tags if tag.startswith('EBY'))
        print(f"  EBX days: {ebx_count}")
        print(f"  EBY days: {eby_count}")
        print(f"  Total: {len(sorted_tags)}")
        print(f"  Test days available: {len(test_eby_days)}")
...
def load_batches(data_dir: str, batch_sz: int):
    ...
    x_fs = sorted(list(dp.glob('X_*.npy')))
    y_fs = sorted(list(dp.glob('y_*.npy')))
...
def get_feat_names(data_dir: str) -> tuple:
    ...
    dp = Path(data_dir)
    xfs = sorted(list(dp.glob('X_*.npy')))
...
def train_model():
    ...
    print("\n[ER] Training Expected Return regression model...")

    # Load all flattened X, R pairs
    train_dir = Path(CONFIG['paths']['train_data_dir'])
    x_files = sorted(train_dir.glob("X_*.npy"))
    r_files = sorted(train_dir.glob("R_*.npy"))

    XR = []
    RR = []
    for xf, rf in zip(x_files, r_files):
        Xtmp = np.load(xf)
        Rtmp = np.load(rf)
        Xtmp = Xtmp.reshape(Xtmp.shape[0], -1)  # flatten
        XR.append(Xtmp)
        RR.append(Rtmp)

    X_all = np.concatenate(XR, axis=0)
    R_all = np.concatenate(RR, axis=0)

    # Fit regression XGBoost
    dtrain_er = xgb.DMatrix(X_all, label=R_all)
    er_params = {"objective": "reg:squarederror", "eta": 0.05, "max_depth": 5}
    bst_er = xgb.train(er_params, dtrain_er, num_boost_round=200)

    bst_er.save_model(CONFIG['paths']['er_model_file'])
    print(f"[ER] Saved ER model → {CONFIG['paths']['er_model_file']}")
...
def prepare_test_data(test_days_input):
    print("="*80)
    ...
    if isinstance(test_days_input, str) and test_days_input.endswith('.txt'):
        test_file = Path(test_days_input)
    ...
    with open(test_file, 'r') as f:
        available_days = [line.strip() for line in f if line.strip()]
    day_numbers = []
    for line in available_days:
        try:
            day_numbers.append(int(line.replace('day', '')))
        except ValueError:
    ...
    if not (isinstance(test_days_input, str) and test_days_input.endswith('.txt')):
        try:
            num_days = int(test_days_input)
            if num_days > len(day_numbers):
                print(f"\nError: Requested {num_days} days but only {len(day_numbers)} available.")
                print("Please train with more days or request fewer test days.")
                return
            day_numbers = day_numbers[:num_days]
            print(f"Processing first {num_days} days from test file")
        except:
            pass
...
class CandleBasedStrategy:
    ...
    def _count_predictions(self):
        ...
        # For EBY: Long=1, Short=2 (aligned with training data)
        long_count = sum(1 for cls in self.current_candle_predictions if cls == 1)
        short_count = sum(1 for cls in self.current_candle_predictions if cls == 2)
        ...
        if desired_position != current_position:
            ...
            if desired_position != 0:
                ...
                if can_open and entry_info is None:
                    # ==== ER-based position sizing ====
                    if 'p' in candle_signals.columns:
                        p_c = candle_signals['p'].mean()
                        b_c = candle_signals['b'].mean()
                    else:
                        p_c = 0.5
                        b_c = 1.0

                    kelly = (p_c * (b_c + 1) - 1) / b_c
                    kelly = max(0, kelly)
                    pos_pct = min(0.5 * kelly, 1.0)   # fractional Kelly

                    shares = (current_capital * pos_pct) / entry_price
                    # ==================================
...
def process_day_with_signals(x_file, idx_file, price_file, csv_path, bst,bst_er, 
    ...
    for batch_idx in range(num_batches):
        ...
        # ==== ADDED: ER regression predictions ====
        # Flatten sequence batch if needed
        if X_batch.ndim == 3:
            X_batch_flat = X_batch.reshape(X_batch.shape[0], -1)
        else:
            X_batch_flat = X_batch

        dtest_er = xgb.DMatrix(X_batch_flat)
        ER_batch = bst_er.predict(dtest_er)   # ER regression model predictions

        # Convert ER → p
        # p = 0.5 + ER / (2 * avg_abs_move)
        avg_abs_move = 0.004   # 0.4% typical absolute move; tune later
        p_batch = 0.5 + ER_batch / (2 * avg_abs_move)
        p_batch = np.clip(p_batch, 0.05, 0.95)

        # Convert ER → b  (simple ratio)
        b_batch = np.abs(ER_batch) / np.maximum(-ER_batch, 1e-6)
        b_batch = np.clip(b_batch, 0.2, 5.0)

        # Store in arrays aligned with samples
        ER_arr[start_idx:end_idx] = ER_batch
        p_arr[start_idx:end_idx]  = p_batch
        b_arr[start_idx:end_idx]  = b_batch
        # ==== END ER regression ====
...
def generate_signals(args):
    """Generate signals for test days and show performance metrics."""
    print("="*80)
    ...
    # Load test days from the test_data_dir
    if args.days: 
    ...
    try:
        ...
        bst_er = xgb.Booster()
        bst_er.load_model(CONFIG['paths']['er_model_file'])
        print(f"✓ Loaded ER model: {CONFIG['paths']['er_model_file']}")
    ...
    data_dirs = CONFIG['paths']['data_dir']
    if isinstance(data_dirs, list):
        data_dir = Path('EBY')
    else:
        data_dir = Path(data_dirs)
    ...
    for day_index in tqdm(test_days, desc="Processing Days"):
        ...
        csv_path = data_dir / f'day{day_index}.csv'
        ...
        try:
            day_trades, day_equity, whipsaw_info, signals_output = process_day_with_signals(
                x_file, idx_file, price_file, csv_path, bst, bst_er,day_index, 
...
def test_pipeline(args):
    ...
    if args.days:
        test_days_input = args.days
    else:
        test_days_input = Path(CONFIG['paths']['train_data_dir']) / CONFIG['paths']['test_days_file']
        if not test_days_input.exists():
            print(f"\nError: Test days file not found at {test_days_input}")
            print("Please run training first or provide --days argument")
            return
    
    print("\n[Step 1/3] Preparing Test Data...")
    prepare_test_data(test_days_input)
...
def load_test_days_for_backtest(filepath: str):
    """Load test days from text file for backtesting."""
    test_days = []
...
def load_signals_for_backtest(day: int, signals_dir: str):
...
def backtest_ebullient_pipeline(args):
    ...
    plots_dir = Path(PLOTS_DIR) / "ebullient_backtest"
    plots_dir.mkdir(exist_ok=True, parents=True)
    ...
    for day in test_days:
        ...
        try:
            current_signals_df, signals_dict = load_signals_for_backtest(day, SIGNALS_DIR)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="EBY.py - Unified Trading System",
    ...
    train_parser.add_argument('--days', required=True,
                             help='Number of days (int) or path to train_days_EBY.txt file')
    train_parser.add_argument('--config', required=True,
                             help='Path to configuration JSON file')
    
    test_parser = subparsers.add_parser('test', help='Test the model')
    test_parser.add_argument('--days', default=None,
                            help='Path to test_days_EBY.txt file or number of days (optional)')
